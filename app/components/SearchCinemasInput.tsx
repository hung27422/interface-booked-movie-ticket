  ...
  return (
    <div>
      <FormControl sx={{ m: 1, width: 340, ...selectStyles }}>
        <InputLabel id="location-select-label">Chọn rạp</InputLabel>
        <Select
          labelId="location-select-label"
          id="location-select"
          value={selectedLocation}
          onChange={handleChange}
          input={<OutlinedInput label="Chọn rạp" />}
          MenuProps={MenuProps}
          sx={selectStyles}
          size="small"
        >
          {cinemas.map((cinema) => (
            <MenuItem key={cinema.code} value={cinema.code}>
              {cinema.title}
            </MenuItem>
          ))}
        </Select>
