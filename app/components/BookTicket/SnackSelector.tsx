...
function SnackSelector({ dataRoom }: SnackSelectorProps) {
  ...
  return (
    <div className="flex flex-col px-4 py-2 bg-gray-800 rounded-lg">
      <div className="grid grid-cols-3 md:grid-cols-8 justify-center items-center text-base md:text-xl text-pink-400 bg-blue-950 p-2 rounded-lg">
        <span className="col-span-1 md:col-span-4 font-bold">COMBO</span>
        <span className="col-span-1 md:col-span-3 mx-auto font-bold">GIÁ TIỀN</span>
        <span className="col-span-1 md:col-span-1 mx-auto font-bold">SỐ LƯỢNG</span>
      </div>
      {dataSnacksByCinema?.map((item) => {
        ...
        return (
          <div
            key={item._id}
            className="grid grid-cols-3 md:grid-cols-8 items-center justify-center border-2 border-[#9400ff] rounded-md py-1 px-2 mt-2"
          >
